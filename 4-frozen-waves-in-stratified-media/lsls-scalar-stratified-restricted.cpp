...
int main()
{
    ...
    // For lossy media, verified only for traditional method!
    ...
    cout << "| LSLS-SCALAR-STRATIFIED-RESTRICTED | 11 MAR 2024 | @JODESARRO |" << endl;
    cout << "A C++ routine to evaluate the scalar field of longitudinally structured light sheets in stratified structure. The beam is structured in a lossy or lossless medium after passing through several media and is restricted to the case where Qm=Q, Lm=L and Nm=N." << endl;
    ...
    static const int N = 30; // It is assumed Nm=N.
    ...
    // Calculation of maximum values for indices n and m of frozen waves.
    ...
    double d [ismax-1] = { 0., 0.005, 0.01 };
    complex<double> nref [ismax] = { 1.0+I*1.4e-7, 1.4+I*1.0e-7, 1.0+I*1.4e-6, 1.4+I*1.0e-6 };
    ...
    //  Set generate_with_B=true to use the coefficient Bnm to generate the beam inside the first medium (Bnm=Anm/Tnm)
    //  or generate_with_B=false to use the traditional coefficient Anm to generate the beam inside the first medium.
    ...
    complex<double> * h = new complex<double> [inmax];
    ...
    else
    {
        for ( int in=0; in<inmax; in++ )
        {
            double n = static_cast<double>( in - N );
            double brnS = Q + 2.0*M_PI*n/L;
            b[in + inmax*(ismax-1)] = complex<double> ( brnS, brnS*imag(nref[ismax-1])/real(nref[ismax-1]) );
            h[in] = sqrt( k0*nref[ismax-1]*k0*nref[ismax-1] - b[in + inmax*(ismax-1)]*b[in + inmax*(ismax-1)] );
            for (int is=0; is<ismax-1; is++)
            {
                b[in + inmax*is] = sqrt( k0*nref[is]*k0*nref[is] - h[in]*h[in] );
            }
    ...
    // Resistant attenuation or compensation parameter inside the last medium.
    ...
    complex<double> nrefmin;
    if (is_purely_real_transverse_wavenumber)
    {
        nrefmin = ( sqrt( real(nref[ismax-1])*real(nref[ismax-1])*(1.0-a*a) + imag(nref[ismax-1])*imag(nref[ismax-1])*(1.0/(a*a)-1.0) ) +I*0. );
    }
    else
    {
        nrefmin = sqrt(1.0-a*a)*( real(nref[ismax-1]) + I*imag(nref[ismax-1]) );
    }
    ...
    if (is_purely_real_transverse_wavenumber)
    {
        ...
        Nmax = floor( L * min( Q - *max_element(kprime, kprime + ismax), k0*real(nref[ismax-1]) - Q )/(2.0*M_PI) );
    }
    else
    {
        double * kprime = new double [ismax];
        for ( int is=0; is<ismax; is++ )
        {
            double kAprimes = real( k0*real(nref[ismax-1])*sqrt(1.0-pow( (k0*real(nref[is]))/(k0*real(nref[ismax-1])), 2)) ); 
            double kBprimes = real( k0*real(nref[ismax-1])*sqrt(1.0-pow( (k0*imag(nref[is]))/(k0*imag(nref[ismax-1])), 2)) ); 
            kprime[is] = max(0.,max(kAprimes,kBprimes));
        }
        Nmax = floor( L * min( Q - *max_element(kprime, kprime + ismax), k0*real(nref[ismax-1]) - Q )/(2.0*M_PI) );
    }
    ...
    // Calculation of axicon angles for generating Bessel beams inside the first medium
    //  in z≤0 where the beams are supposed to be generated.
    //  The θnm=θn is a consequence of hnm=hn.
    complex<double> theta_pN = asin( (h[inmax-1])/(k0*nref[0]) );
    cout << "Calculated axicon angle inside first medium (z<=0) for n=+N: ";
    cout << theta_pN << " rad = " << (360./(2.0*M_PI))*theta_pN << " degree" << endl;
    complex<double> theta_mN = asin( (h[0])/(k0*nref[0]) );
    cout << "Calculated axicon angle inside first medium (z<=0) for n=-N: ";
    cout << theta_mN << " rad = " << (360./(2.0*M_PI))*theta_mN << " degree" << endl;

    complex<double> theta_pN1 = asin( (h[inmax-1])/(k0*nref[1]) );
    cout << "Calculated axicon angle inside 2nd medium for n=+N: ";
    cout << theta_pN1 << " rad = " << (360./(2.0*M_PI))*theta_pN1 << " degree" << endl;
    complex<double> theta_mN1 = asin( (h[0])/(k0*nref[1]) );
    cout << "Calculated axicon angle inside 2nd medium for n=-N: ";
    cout << theta_mN1 << " rad = " << (360./(2.0*M_PI))*theta_mN1 << " degree" << endl;

    complex<double> theta_pN2 = asin( (h[inmax-1])/(k0*nref[2]) );
    cout << "Calculated axicon angle inside 3rd medium for n=+N: ";
    cout << theta_pN2 << " rad = " << (360./(2.0*M_PI))*theta_pN2 << " degree" << endl;
    complex<double> theta_mN2 = asin( (h[0])/(k0*nref[2]) );
    cout << "Calculated axicon angle inside 3rd medium for n=-N: ";
    cout << theta_mN2 << " rad = " << (360./(2.0*M_PI))*theta_mN2 << " degree" << endl;
    
    complex<double> theta_pN3 = asin( (h[inmax-1])/(k0*nref[3]) );
    cout << "Calculated axicon angle inside 4th medium for n=+N: ";
    cout << theta_pN3 << " rad = " << (360./(2.0*M_PI))*theta_pN3 << " degree" << endl;
    complex<double> theta_mN3 = asin( (h[0])/(k0*nref[3]) );
    cout << "Calculated axicon angle inside 4th medium for n=-N: ";
    cout << theta_mN3 << " rad = " << (360./(2.0*M_PI))*theta_mN3 << " degree" << endl;
